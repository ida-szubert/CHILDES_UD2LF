...
parser.add_option("-o", "--output", dest="out_file", default="./conll",
                  help="destination file for preprocessed UD annotation")
...
def get_childes_sent_dict():
    ...
    for ch_file in childes_files:
        with open(join(opts.out_file, ''.join([ch_file.rstrip('xml'), 'txt'])), "w") as out_file:
            data = CHILDESCorpusReader(opts.childes, ch_file)
            adults = set(flatten([file_inf.keys() for file_inf in data.participants()]))
            adults.remove('CHI')
            tagged_sents = en.tagged_sents(data.fileids()[0], speaker=adults, stem=True)
            sents = en.sents(data.fileids()[0], speaker=adults)
            for i,(POSwords, stems_and_words) in enumerate(zip(tagged_sents,sents)):
                ...
                try:
                    sentence_buffer = parse_dict[analysed_sent]
                except KeyError:
                    try:
                        sentence_buffer = parse_dict[other_sent]
                    except KeyError:
                        pass
                if sentence_buffer:
                    for i,(wPOS, w_o) in enumerate(zip(sentPOS, split_sent_list)):
                        sentence_buffer[i][1] = wPOS
                        sentence_buffer[i][2] = w_o
                    for s_line in sentence_buffer:
                         out_file.write('\t'.join(s_line)+'\n')
                    out_file.write('\n')
    ...
    for line in open(in_file, "r"):
        ...
        #     pass
            ...
            if valid_parse:
                ...
                sents = set([sentence, another_sentence, other_sentence])
                for s in sents:
...
parse_dict = preprocess_annotation(opts.out_file, opts.in_file, opts.invalid_file)
get_childes_sent_dict()
